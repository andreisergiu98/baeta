...
import { decodeBase64Url, encodeBase64Url } from './base64-url.ts';
...
test('Round trip: encode -> decode should return original string', (t) => {
...
test('Round trip: should handle empty string', (t) => {
	const original = '';
	const encoded = encodeBase64Url(original);
	const decoded = decodeBase64Url(encoded);
	t.is(decoded, original);
