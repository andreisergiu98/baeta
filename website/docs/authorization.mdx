...
export const authExt = authExtension<Context>(
    async (ctx) => {
        ...
        return {
            isPublic: true,
            isLoggedIn: async () => { // But you can also use scope loaders, which will be resolved lazily, when needed
                if (!ctx.userId) {
                    throw new UnauthenticatedError();
                }
                return true;
            },
            hasAccess: (access: string) => { // Scope loaders are also required for non-boolean scopes
...
3. Point Baeta to registered extensions
...
Query.user.$postAuth((params, result) => {
  if (result && result.id === params.ctx.userId) {
    return {}; // Allow access if user is requesting their own data
  }
...
Query.user.$auth(async (params) => {
  const user = await db.user.findFirst({ // Extra database query
