...
type StoreOptions = {
  ...
  warnOnDuplicateLoader?: boolean; // Warn when a loader is set on a context that already has one. Default: true
};
```

Setting a loader on a context that already has one keeps the existing loader and logs a warning. If your setup intentionally registers the same loader multiple times per request (e.g. from a middleware that runs per field), disable the warning with `warnOnDuplicateLoader: false`.
