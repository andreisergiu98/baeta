...
The cache system has been fully rewritten and split out of the GraphQL framework into a standalone `@baeta/cache` package. It no longer depends on `@baeta/core` or `graphql` and can be used on its own.
...
const updateUserMutation = Mutation.updateUser
  ...
  .resolve(({ args }) => db.user.update({ where: { id: args.where.id }, data: args.data }));
```
  ...
  .withQueries({
    findUserPhotos: defineQuery({
      ...
      onInsert(items, helpers) {
        return helpers.invalidateByArgs(items.map((item) => ({ userId: item.userId })));
      },
      onDelete(pairs, helpers) {
        return helpers.invalidateByArgs(
          pairs.flatMap((p) => (p.previous ? [{ userId: p.previous.userId }] : [])),
        );
      },
    }),
  })
