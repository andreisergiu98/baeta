...
Compute the cost from `ctx` or `args`. For paginated fields this is the recommended setup, deriving the multiplier from the page-size argument makes the score track the amount of work actually requested:
...
:::warning
The default list multiplier is a flat constant, it is **not** derived from paging arguments. `users(first: 100000)` scores the same as `users(first: 1)` unless the field sets its multiplier from `args` (see [Dynamic rules](#dynamic-rules)).
:::
...
query {
  users(first: 100) {
    # complexity: 1 * 10 (default multiplier)
